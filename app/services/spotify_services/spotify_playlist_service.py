...
# =============================================================================
# Bu modül, Spotify çalma listeleri ile ilgili çeşitli işlemleri yöneten
# bir servis sınıfı içerir. API etkileşimleri için SpotifyApiService'i kullanır.
#
# =============================================================================
# İÇİNDEKİLER
# =============================================================================
...
#
# 2.0  SPOTIFY ÇALMA LİSTESİ SERVİS SINIFI (SpotifyPlaylistService)
#
#      2.1  TEMEL METOTLAR (CORE METHODS)
#           2.1.1. __init__(api_service=None)
#                  : Başlatıcı metot.
#
#      2.2  ÇALMA LİSTESİ VERİSİ ALMA (PLAYLIST DATA RETRIEVAL)
#           2.2.1. get_user_playlists(username, limit=50, offset=0)
#                  : Kullanıcının kendi çalma listelerini alır.
#           2.2.2. get_playlist(username, playlist_id)
#                  : Belirli bir ID'ye sahip çalma listesini alır.
#           2.2.3. get_playlist_tracks(username, playlist_id, limit=100, offset=0)
#                  : Bir çalma listesindeki parçaları alır.
#           2.2.4. get_featured_playlists(username, limit=20, offset=0)
#                  : Spotify'ın öne çıkan çalma listelerini alır.
#           2.2.5. get_category_playlists(username, category_id, limit=20, offset=0)
#                  : Belirli bir kategoriye ait çalma listelerini alır.
#           2.2.6. get_categories(username, limit=20, offset=0)
#                  : Mevcut Spotify kategori listesini alır.
#
#      2.3  ÇALMA LİSTESİ YÖNETİMİ (PLAYLIST MANAGEMENT)
#           2.3.1. create_playlist(username, user_id, name, public, collaborative, description)
#                  : Yeni bir çalma listesi oluşturur.
#           2.3.2. update_playlist_details(username, playlist_id, name, public, collaborative, description)
#                  : Bir çalma listesinin detaylarını günceller.
#           2.3.3. follow_playlist(username, playlist_id, public=True)
#                  : Bir çalma listesini takip eder.
#           2.3.4. unfollow_playlist(username, playlist_id)
#                  : Bir çalma listesini takipten çıkar.
#
#      2.4  ÇALMA LİSTESİ ÖĞELERİNİ YÖNETME (PLAYLIST ITEM MANAGEMENT)
#           2.4.1. add_items_to_playlist(username, playlist_id, uris, position=None)
#                  : Bir çalma listesine parça(lar) ekler.
#           2.4.2. remove_items_from_playlist(username, playlist_id, uris)
#                  : Bir çalma listesinden parça(ları) kaldırır.
#           2.4.3. reorder_playlist_items(username, playlist_id, range_start, insert_before, range_length, snapshot_id)
#                  : Çalma listesindeki parçaların sırasını değiştirir.
#
#      2.5  ARAMA VE FORMATLAMA (SEARCH & FORMATTING)
#           2.5.1. search_items(username, query, item_types, limit, offset, market)
#                  : Spotify'da parça, albüm, sanatçı veya çalma listesi arar.
#           2.5.2. format_playlist_for_display(playlist_data)
#                  : Çalma listesi verisini gösterim için formatlar.
#           2.5.3. format_track_for_display(track_data)
#                  : Parça verisini gösterim için formatlar.
# =============================================================================
...
class SpotifyPlaylistService:
    ...
    def __init__(self, api_service: Optional[SpotifyApiService] = None):
        """
        2.1.1. SpotifyPlaylistService sınıfının başlatıcı metodu.
        """
    ...
    def get_user_playlists(self, username: str, limit: int = 50, offset: int = 0) -> Optional[Dict[str, Any]]:
        """
        2.2.1. Belirtilen kullanıcının Spotify çalma listelerini alır.
        """
        try:
            limit = max(1, min(limit, 50))
            offset = max(0, offset)
            return self.api_service._make_api_request(username=username, endpoint=f"me/playlists?limit={limit}&offset={offset}")
    ...
    def get_playlist(self, username: str, playlist_id: str) -> Optional[Dict[str, Any]]:
        """
        2.2.2. Belirli bir ID'ye sahip çalma listesinin detaylarını alır.
        """
        try:
            if not playlist_id or not playlist_id.strip():
                self.logger.error("get_playlist için geçersiz playlist_id.")
                return None
            return self.api_service._make_api_request(username=username, endpoint=f"playlists/{playlist_id.strip()}")
        except Exception as e:
    ...
    def get_playlist_tracks(self, username: str, playlist_id: str, limit: int = 100, offset: int = 0) -> Optional[Dict[str, Any]]:
        """
        2.2.3. Belirli bir çalma listesindeki parçaları alır.
        """
        try:
            if not playlist_id or not playlist_id.strip():
            ...
            offset = max(0, offset)
            return self.api_service._make_api_request(username=username, endpoint=f"playlists/{playlist_id.strip()}/tracks?limit={limit}&offset={offset}")
        except Exception as e:
    ...
    def get_featured_playlists(self, username: str, limit: int = 20, offset: int = 0) -> Optional[Dict[str, Any]]:
        """
        2.2.4. Spotify'ın öne çıkan çalma listelerini alır.
        """
        try:
            limit = max(1, min(limit, 50))
            offset = max(0, offset)
            return self.api_service._make_api_request(username=username, endpoint=f"browse/featured-playlists?limit={limit}&offset={offset}")
        except Exception as e:
    ...
    def get_category_playlists(self, username: str, category_id: str, limit: int = 20, offset: int = 0) -> Optional[Dict[str, Any]]:
        """
        2.2.5. Belirli bir kategoriye ait çalma listelerini alır.
        """
        try:
            ...
            limit = max(1, min(limit, 50))
            offset = max(0, offset)
            return self.api_service._make_api_request(username=username, endpoint=f"browse/categories/{category_id.strip()}/playlists?limit={limit}&offset={offset}")
        except Exception as e:
    ...
    def get_categories(self, username: str, limit: int = 20, offset: int = 0) -> Optional[Dict[str, Any]]:
        """
        2.2.6. Spotify'da mevcut olan müzik kategorilerini alır.
        """
        try:
            limit = max(1, min(limit, 50))
            offset = max(0, offset)
            return self.api_service._make_api_request(username=username, endpoint=f"browse/categories?limit={limit}&offset={offset}")
        except Exception as e:
    ...
    def create_playlist(self, username: str, user_id: str, name: str, public: bool = True, collaborative: bool = False, description: str = "") -> Optional[Dict[str, Any]]:
        """
        2.3.1. Belirtilen Spotify kullanıcısı için yeni bir çalma listesi oluşturur.
        """
        try:
            ...
            payload: Dict[str, Union[str, bool]] = {
                "name": name.strip(), "public": public, "collaborative": collaborative, "description": description.strip()
            }
            if collaborative:
                payload["public"] = False

            return self.api_service._make_api_request(username=username, endpoint=f"users/{user_id.strip()}/playlists", method="POST", data=payload)
        except Exception as e:
            self.logger.error(f"Çalma listesi ('{name}') oluşturulurken hata: {e}", exc_info=True)
            return None

    def update_playlist_details(self, username: str, playlist_id: str, name: Optional[str] = None, public: Optional[bool] = None, collaborative: Optional[bool] = None, description: Optional[str] = None) -> bool:
        """
        2.3.2. Mevcut bir çalma listesinin detaylarını günceller.
        """
        try:
            if not playlist_id or not playlist_id.strip():
            ...
            response = self.api_service._make_api_request(username=username, endpoint=f"playlists/{playlist_id.strip()}", method="PUT", data=payload)
    ...
    def follow_playlist(self, username: str, playlist_id: str, public: bool = True) -> bool:
        """
        2.3.3. Kullanıcının belirtilen çalma listesini takip etmesini sağlar.
        """
        try:
            if not playlist_id or not playlist_id.strip():
            ...
            response = self.api_service._make_api_request(username=username, endpoint=f"playlists/{playlist_id.strip()}/followers", method="PUT", data={"public": public})
            return response is not None and response.get("status") == "success"
        except Exception as e:
    ...
    def unfollow_playlist(self, username: str, playlist_id: str) -> bool:
        """
        2.3.4. Kullanıcının belirtilen çalma listesini takipten çıkmasını sağlar.
        """
        try:
            if not playlist_id or not playlist_id.strip():
            ...
            response = self.api_service._make_api_request(username=username, endpoint=f"playlists/{playlist_id.strip()}/followers", method="DELETE")
            return response is not None and response.get("status") == "success"
        except Exception as e:
    ...
    def add_items_to_playlist(self, username: str, playlist_id: str, uris: List[str], position: Optional[int] = None) -> bool:
        """
        2.4.1. Belirtilen çalma listesine bir veya daha fazla parça ekler.
        """
        try:
            ...
            response = self.api_service._make_api_request(username=username, endpoint=f"playlists/{playlist_id.strip()}/tracks", method="POST", data=payload)
    ...
    def remove_items_from_playlist(self, username: str, playlist_id: str, uris: List[str]) -> bool:
        """
        2.4.2. Belirtilen çalma listesinden bir veya daha fazla parçayı kaldırır.
        """
        try:
            if not playlist_id or not playlist_id.strip() or not uris:
            ...
            response = self.api_service._make_api_request(username=username, endpoint=f"playlists/{playlist_id.strip()}/tracks", method="DELETE", data=payload)
            return response is not None and "snapshot_id" in response
        except Exception as e:
    ...
    def reorder_playlist_items(self, username: str, playlist_id: str, range_start: int, insert_before: int, range_length: int = 1, snapshot_id: Optional[str] = None) -> bool:
        """
        2.4.3. Bir çalma listesindeki bir veya daha fazla parçanın sırasını değiştirir.
        """
        try:
            if not all([isinstance(range_start, int), range_start >= 0, isinstance(insert_before, int), insert_before >= 0, isinstance(range_length, int), range_length >= 1]):
            ...
            response = self.api_service._make_api_request(username=username, endpoint=f"playlists/{playlist_id.strip()}/tracks", method="PUT", data=payload)
            return response is not None and "snapshot_id" in response
        except Exception as e:
    ...
    def search_items(self, username: str, query: str, item_types: List[str] = ['track'], limit: int = 20, offset: int = 0, market: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """
        2.5.1. Spotify'da belirtilen sorgu ile arama yapar.
        """
        try:
            ...
            if not types_str: return {"error": "Geçersiz arama türleri."}
    ...
    def format_playlist_for_display(self, playlist_data: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        """
        2.5.2. Ham çalma listesi verisini arayüz için uygun formata dönüştürür.
        """
    ...
    def format_track_for_display(self, track_data: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        """
        2.5.3. Ham parça verisini arayüz için uygun formata dönüştürür.
        """
